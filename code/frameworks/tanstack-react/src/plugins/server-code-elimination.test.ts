...
`;
  ...
  describe('executeServerFunctions option', () => {
    ...
    /**
     * The real `createMiddleware` builder exposes `inputValidator` and no
     * `validator` at all, so a chain calling `.validator()` only works because
     * this strip deletes the call before the browser sees it. Keeping it under
     * the option would throw "createMiddleware(...).validator is not a
     * function" at module evaluation and take down the whole story file, so
     * this one strip stays unconditional while the phases beside it do not.
     */
    it('still strips a createMiddleware validator when the option is on', async () => {
      const code = [
        `import { createMiddleware } from '@tanstack/react-start';`,
        `export const m = createMiddleware()`,
        `  .validator((v) => v)`,
