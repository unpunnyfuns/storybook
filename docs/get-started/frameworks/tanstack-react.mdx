...
In practice, this means you can usually render TanStack Start components directly. A `createServerFn()` chain still exports a mock function that you can observe and override in stories and tests, and a call you do not override runs the real chain around your handler: the middleware you attached to that function and the validator you gave it. See [What happens when you do not mock a server function](#what-happens-when-you-do-not-mock-a-server-function).
...
**Breaking change: unmocked server functions now run their chain**

Earlier versions replaced a `createServerFn()` chain with a mock that discarded its middleware and its validator, so a call you did not override did nothing at all. Storybook now delegates to TanStack's own builder, so the chain runs. Two things to check when you upgrade:

- The `client` phase of every middleware attached with `.middleware([...])` runs on each unmocked call, so its side effects happen in stories.
- The validator passed to `.validator()` or `.inputValidator()` runs, so a story that calls a server function with input the validator rejects now fails instead of quietly reaching the handler.

Stories that already override the function are unaffected, because [mocking short-circuits the chain](#mocking-short-circuits-the-chain).

The same release also changes cookie scope to match the real runtime: `setCookie()` writes to the response and `getCookie()` reads from the request, so a value you set is no longer visible to a `getCookie()` later in the same story.

</Callout>

Storybook replaces the transport rather than the builder, so the chain a story runs is the chain your app declared, in the order it declared it. For a call you do not override, this runs:
...
- The validator the server function itself declares, through `.validator()` or `.inputValidator()`.

Your `.handler()` body and the `server` and `inputValidator` phases of your `createMiddleware()` chains are not in the Storybook build by default, so an unmocked call resolves to `undefined` rather than to whatever your handler would have returned. What changed is everything around that. To keep them in the build, see [Running your own handler code](#running-your-own-handler-code).
...
The serialization is not skipped. The payload and the result are encoded the way the real transport encodes them, so a story cannot hand a handler a value a real request could not carry, and a handler never receives the object the story passed, only a copy of it. `FormData` is the exception the real client also makes: it is passed through as `FormData` instead of being serialized.

Two kinds of return value bypass serialization in a story exactly as they do on a real server:

- A `Response` is handed back to the caller untouched, with the `x-tss-raw` header set. This includes a `Response` your handler threw, which arrives as a resolved value rather than a rejection.
- A `redirect()` is thrown, so a story observes it as a rejected call.
...
Middleware attached per function with `.middleware([...])` is not affected by any of this, and neither is a server function's own validator.
...
By default the framework removes server-only code from the build before it can reach the browser. For server functions that means the `.handler()` body is replaced with a no-op spy, and the `server` and `inputValidator` phases of `createMiddleware()` chains are dropped.

The experimental `executeServerFunctions` framework option keeps them instead, so the chain a story runs includes your own handler and your middleware's server phase:
