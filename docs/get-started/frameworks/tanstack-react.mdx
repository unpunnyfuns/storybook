...
Whether navigation is performed in this story, or only recorded. By default, `Link`, `Navigate`, `useNavigate()`, and `useRouter().navigate` record the attempt on the `onNavigate` spy (see [`@storybook/tanstack-react/spies`](#storybooktanstack-reactspies)) and leave the story on screen. Set it to `true` and all four still record, then navigate for real, so the story renders whatever the destination route renders.

Turn it on for a story that asserts on where navigation lands. Navigating to a route outside the story's mounted route tree renders a not found, the same as it would in the app.
