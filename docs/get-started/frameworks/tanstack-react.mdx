...
The serialization is not skipped. The payload and the result are encoded the way the real transport encodes them, so a story cannot hand a handler a value a real request could not carry, and a handler never receives the object the story passed, only a copy of it. `FormData` is the exception the real client also makes: it is copied rather than serialized, so `File` and `Blob` entries inside it are shared with the story instead of being rebuilt.
...
- A `redirect()` your handler returns or throws reaches the caller as a rejected call carrying that redirect. A redirect thrown from a middleware's `client` phase reaches the caller the same way, and that one does happen in a default build.
...
The experimental `executeServerFunctions` framework option keeps your handler body and your middleware's `server` phase, so the chain a story runs includes your own server code:
...
Validators are outside what the option covers. A server function's `.validator()` or `.inputValidator()` is removed from the build whether it is on or off, so the input your handler sees in a story is unvalidated either way.
...
Experimental. Keeps `createServerFn().handler()` bodies and the `server` phase of `createMiddleware()` chains in the Storybook build instead of stripping them, so a story can run your own server code. It does not make that code browser-safe, and it does not restore validators. See [Running your own handler code](#running-your-own-handler-code).
